"""Tests for the Teacher Transformer model."""
import pytest
import torch
...
@pytest.fixture(params=[
    ...
    # Test case 2: Don't freeze encoders, hf features
    {
        'hidden_size': 512,
...
def model(model_config):
    return TeacherTransformer(**model_config)

def test_teacher_init(model, model_config):
    """Test if we can initialize a TeacherTransformer model with different configs."""
...
def test_model_save_load(tmp_path, model, model_config):
    ...
    new_model = TeacherTransformer(**model_config)
